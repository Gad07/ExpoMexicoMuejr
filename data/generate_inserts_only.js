...
sql += `-- =============================================================================\n`;
sql += `-- EXPO MÉXICO MUJER - STANDALONE INSERTS ONLY BACKUP\n`;
...
try {
  ...
  sql += `-- INSERTS: TARJETAS DIGITALES\n`;
  cards.forEach((c, idx) => {
...
fs.writeFileSync(path.join(dataDir, 'inserts.sql'), sql, 'utf-8');
console.log('inserts.sql generated successfully! Total size:', sql.length, 'bytes');
