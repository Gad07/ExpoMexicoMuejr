...
import { Plus, Trash2, Save, X, Globe, Mail, User, Image, Check, AlertCircle, Medal, Search } from 'lucide-react';
...
const EMPTY_SPONSOR: Sponsor = {
  ...
  tier: { es: '', en: '', fr: '' },
  name: '',
  logo: 'https://placehold.co/200x80/e0e0e0/555555?text=Logo',
...
export default function AdminPatrocinadores() {
  ...
  const updateField = (field: keyof Sponsor, value: any) => {
    if (!editing) return;
    setEditing({ ...editing, [field]: value });
  };

  const setLocField = (field: keyof Sponsor, lang: string, value: string) => {
    if (!editing) return;
  ...
  const handleNew = () => {
    setEditing({ ...EMPTY_SPONSOR, id: Date.now().toString(), gallery: [''] });
    setIsNew(true);
    setMessage(null);
  ...
  const handleEdit = (sponsor: Sponsor) => {
    setEditing({
      ...sponsor,
      gallery: sponsor.gallery && sponsor.gallery.length > 0 ? [...sponsor.gallery] : [''],
    });
    setIsNew(false);
    setMessage(null);
    setTimeout(() => editorRef.current?.scrollIntoView({ behavior: 'smooth' }), 100);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setMessage({ type: 'error', text: data.error || 'Error al guardar patrocinador' });
        setSaving(false);
        return;
      }

      setMessage({ type: 'success', text: isNew ? 'Patrocinador creado exitosamente' : 'Patrocinador actualizado exitosamente' });
      await fetchSponsors();
      if (isNew) {
        setEditing(null);
        setIsNew(false);
  ...
  const filtered = sponsors.filter(s => 
    ...
    getLocVal(s.tier, 'es').toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', minHeight: 'calc(100vh - 72px)' }}>
      ...
      <div style={{ background: '#fff', borderRight: '1px solid rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 72px)', position: 'sticky', top: '72px' }}>
        <div style={{ padding: '24px 24px 16px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
          <h1 style={{ margin: '0 0 16px', fontSize: '1.4rem', fontWeight: 800, color: '#002E51', lineHeight: 1.1 }}>Patrocinadores</h1>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '32px 20px', color: '#888', fontSize: '0.85rem' }}>No se encontraron patrocinadores.</div>
          ) : (
              ...
              {filtered.map(s => {
                ...
                return (
                  <div key={s.id} style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    >
                      <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: '#fff', border: '1px solid #eee', overflow: 'hidden', flexShrink: 0, padding: '4px' }}>
                        {s.logo ? (
                          <img src={s.logo} alt={s.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                        ) : (
                          <Medal size={18} color="#aaa" style={{ margin: '4px' }} />
                        )}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#002E51', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</div>
                        <div style={{ fontSize: '0.75rem', color: '#666', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{getLocVal(s.tier, 'es')}</div>
                      </div>
                    </button>
                    <button
        ...
        {!editing ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.05)', borderRadius: '24px', padding: '48px 32px', maxWidth: '420px', textAlign: 'center', boxShadow: '0 10px 30px rgba(0,25,76,0.02)' }}>
              ...
              <h3 style={{ margin: '0 0 8px', fontSize: '1.2rem', fontWeight: 800, color: '#002E51' }}>Gestión de Patrocinadores</h3>
              <p style={{ margin: 0, fontSize: '0.85rem', color: '#666', lineHeight: 1.6 }}>Selecciona un patrocinador de la lista para editar sus datos, o haz clic en <strong>"+ Agregar Patrocinador"</strong> para registrar uno nuevo.</p>
            </div>
          </div>
        ) : (
          <div style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
            ...
            {showDeleteConfirm && (
              <div style={{ padding: '16px', borderRadius: '8px', marginBottom: '20px', background: '#fff', border: '1px solid rgba(228,0,124,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: '0.9rem', color: '#111' }}>¿Eliminar este patrocinador?</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '32px' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
                  <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700, color: '#111' }}>{isNew ? 'Nuevo Patrocinador' : 'Editar Patrocinador'}</h3>
                  <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.05)', padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, marginBottom: '6px', color: '#002E51' }}>Nombre de la Empresa</label>
                  ...
                  <div style={{ border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '16px', background: '#fafafa' }}>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, marginBottom: '6px', color: '#002E51' }}>Nivel de Patrocinio ({langTab.toUpperCase()})</label>
                      <input type="text" value={getLocVal(editing.tier, langTab)} onChange={e => setLocField('tier', langTab, e.target.value)} placeholder="Ej. Socio Diamante" style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid rgba(0,0,0,0.1)', fontSize: '0.85rem', boxSizing: 'border-box' }} />
                    </div>

                    <div style={{ marginBottom: '16px' }}>
                      <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, marginBottom: '6px', color: '#002E51' }}>Biografía corta / Por qué apoyan ({langTab.toUpperCase()})</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, marginBottom: '6px', color: '#002E51' }}>Descripción Comercial ({langTab.toUpperCase()})</label>
              ...
              <div>
                ...
                <div style={{ position: 'sticky', top: '24px' }}>
                  <div style={{ background: '#fff', borderRadius: '24px', overflow: 'hidden', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 20px 40px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '16px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#002E51', background: '#002E5112', padding: '4px 12px', borderRadius: '100px' }}>
                        {getLocVal(editing.tier, langTab) || 'Nivel de Patrocinio'}
                      </span>
                    </div>
                    ...
                    <div style={{ padding: '0 24px 32px', position: 'relative', textAlign: 'center', marginTop: '16px' }}>
                      ...
                      <h3 style={{ margin: '0 0 4px', fontSize: '1.4rem', fontWeight: 800, color: '#002E51', lineHeight: 1.1 }}>{editing.name || 'Nombre del Patrocinador'}</h3>
