...
export default function NoticiaDetailClient({ noticia }: { noticia: Noticia }) {
  ...
  return (
    <div className="vogue-page">
      ...
      {noticia.author ? (
        <AuthorProfileTemplate noticia={noticia} language={language} />
      ) : noticia.videoUrl ? (
        <CinematicVideoTemplate noticia={noticia} language={language} />
      ) : noticia.images && noticia.images.length > 0 ? (
        <EditorialGalleryTemplate noticia={noticia} language={language} />
      ) : (
        <StandardArticleTemplate noticia={noticia} language={language} />
      )}
    </div>
  );
}
...
/* ──────────────────────────────────────────────────────── */
function StandardArticleTemplate({ noticia, language }: { noticia: Noticia; language: 'es' | 'en' | 'fr' }) {
  return (
    <div className="vogue-article">
      {/* Left Column */}
      <div className="vogue-sidebar">
        <span className="vogue-category">{noticia.category[language]}</span>
        <p className="vogue-excerpt">{noticia.excerpt[language]}</p>
      </div>

      {/* Right Column */}
      <div className="vogue-body">
        <p>{noticia.content[language] || noticia.excerpt[language]}</p>
        <p>
...
/* ────────────────────────────────────────────────────────── */
function EditorialGalleryTemplate({ noticia, language }: { noticia: Noticia; language: 'es' | 'en' | 'fr' }) {
  return (
    <div>
      <div className="vogue-article">
        {/* Left Column */}
        <div className="vogue-sidebar">
          <span className="vogue-category">{noticia.category[language]} // GALLERY</span>
          <p className="vogue-excerpt">{noticia.excerpt[language]}</p>
        </div>

        {/* Right Column */}
        <div className="vogue-body">
          <p className="vogue-excerpt" style={{ color: '#111', fontSize: '1.3rem' }}>{noticia.excerpt[language]}</p>
          <p>{noticia.content[language]}</p>
          ...
          <div className="editorial-images-column">
            <div className="editorial-img-container offset-left">
              <img src={noticia.image} alt={noticia.title[language]} loading="lazy" width="600" height="400" />
              <div className="editorial-img-caption">Imagen Principal: {noticia.title[language]}</div>
            </div>

            {noticia.images && noticia.images.length > 1 && (
              <div className="editorial-img-container offset-right">
                <img src={noticia.images[1]} alt={noticia.title[language]} loading="lazy" width="600" height="400" />
                <div className="editorial-img-caption">Imagen Secundaria: {noticia.title[language]}</div>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="collage-inner">
            {noticia.images.map((img, idx) => (
              <div className="collage-item" key={idx}>
                <img src={img} alt={`Collage ${idx}`} loading="lazy" width="600" height="800" />
              </div>
            ))}
          </div>
        </div>
...
/* ────────────────────────────────────────────────────────── */
function CinematicVideoTemplate({ noticia, language }: { noticia: Noticia; language: 'es' | 'en' | 'fr' }) {
  ...
  return (
    <div>
      <div className="vogue-article">
        {/* Left Column */}
        <div className="vogue-sidebar">
          <span className="vogue-category">{noticia.category[language]} // FILM</span>
          <p className="vogue-excerpt">{noticia.excerpt[language]}</p>
...
/* ────────────────────────────────────────────────────────── */
function AuthorProfileTemplate({ noticia, language }: { noticia: Noticia; language: 'es' | 'en' | 'fr' }) {
  ...
  return (
    <div>
      <div className="vogue-article">
        {/* Left Column */}
        <div className="vogue-sidebar">
          <span className="vogue-category">{noticia.category[language]} // PROFILE</span>
          <p className="vogue-excerpt">{noticia.excerpt[language]}</p>
